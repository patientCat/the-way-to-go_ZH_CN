...
```go
import "./pack1" // 注释：这里导入的是包pack1所在的目录
```
...
如果包 pack1 和我们的程序在同一路径下，我们可以通过 `"import ./"` 这样的方式来引入，但这不被视为一个好的方法。
如果编译的话会报这个错误
```
main.go:3:8: found packages main (test_package.go) and pack (pack1.go) in 你的路径
```
